    ...
    for day in datestamps:
        for traceName in traceNames:
            ...
            for i in tqdm(range(len(temp)-1), desc=traceName, ncols=100, ascii=' =', bar_format='{l_bar}{bar}|'):
                ...
                span["duration"] = row[2]
                span['cmdb_id'] = row[7]
                span["db"] = None if length < 10 else row[-1].replace(
                    '"', "").rstrip()
                # 通过其他文件获取其他 KPIs
                # span["KPIs"] = getKPIs(
            ...
            print("文件"+traceName+"_"+day+".csv "+"合并完毕,共花费 "+str(time_spend)+"S")
            # break
        break
...
def saveJson(res: dict):
    ...
    with open(p, 'w') as f:
        ...
        for i in tqdm(range(length), desc="保存数据中", ncols=100, ascii=' #', bar_format='{l_bar}{bar}|'):
            #? 保存数据时同时 得到 KPIs
            key, val = items.pop(0)
            graph = Trace.generateGraph(val)
            for k,v in val.items():
                if k!="startTime":
                    v['KPIs'] = getKPIs(int(v["timestamp"]), v['cmdb_id'], v["db"])
